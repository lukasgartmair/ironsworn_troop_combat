...
def rnd_move():
    return (random.choice(list(units)), random.choice(list(units)), random.choices(combat_style, weights=ws)[0],random.choices(list(advantage_dict), weights=balancing_weights, k=1)[0])
...
for i in range(iterations):
    ...
    while combat_mod is None:
        ...
        if combat_mod is not None:
            ...
            print("relevant_attacker_ability: {} : {}".format(attacker_attribute, attacker_ability))
            print("relevant_defender_ability: {} : {}".format(defender_attribute, defender_ability))
            attribute_mod = apply_ability_mod(attacker_ability - defender_ability)
            print("Ability mod calculation: {} : {} - {} : {} = {}".format(attacker_attribute, attacker_ability, defender_attribute, defender_ability, attacker_ability - defender_ability))
            print()
            print("Initial: {}, Applied: {}".format(attacker_ability - defender_ability, attribute_mod))
            print()
            print("class difference {} vs. {} in {} is {}".format(my_units, defender_units, results[2], combat_mod))
            print()
            print("Modifiers: class vs class: {}, numbers difference: {}, ability difference {}".format(combat_mod,advantage, attribute_mod))
            print()
            print("Dice Roll")
            my_action_die = d6()
            enemy_dice = d10(), d10()
