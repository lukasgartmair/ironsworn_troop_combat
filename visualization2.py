...
fig, axs = plt.subplots(
    nrows=len(roles), ncols=1, figsize=(12, 6 * len(roles))
)  # Adjust size for multiple subplots
...
for r, role in enumerate(roles):
    ...
    for i, modifier in enumerate(sorted(df["ability_modifier"].unique())):
        ...
        norm = plt.Normalize(
            vmin=sub_data["Miss_Percentage"].min(),
            vmax=sub_data["Miss_Percentage"].max(),
        )
        ...
        ax.bar(
            ...
            + (i - 1) * width
            + r * width * 2
            + x * space_between,  # Offset for each role + space between categories
            sub_data["Miss_Percentage"],
            width,
            label=(
                f"{role.title()} - Miss (Mod {modifier})" if r == 0 and i == 0 else ""
            ),
        )
        ax.bar(
            ...
            label=(
                f"{role.title()} - Weak Hit (Mod {modifier})"
                if r == 0 and i == 0
                else ""
            ),
        )
        ax.bar(
            ...
            label=(
                f"{role.title()} - Strong Hit (Mod {modifier})"
                if r == 0 and i == 0
                else ""
            ),
        )
    ...
    ax.set_xticks(
        x + (len(roles) - 1) * width * 2 / 2
    )  # Adjust xticks to center the categories
