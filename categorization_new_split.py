...
for idx in mapped_df.index:
    ...
    action_die_combination = {(row['miss (Act_Die)'], row['strong_hit (Act_Die)'], row['weak_hit (Act_Die)']) : ("Action Die", idx)}
    challnge_dice_combination = {(row['miss (Split)'], row['strong_hit (Split)'], row['weak_hit (Split)']): ("Challenge Dice", idx)}
...
for entry in all_combinations:
    for (outcome, mod_value) in entry.items():
        ...
        modified_die.append(mod_value[0])  # Extracting the die type ("Action" or "Chllenge")
