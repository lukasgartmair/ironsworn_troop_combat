...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 19 10:47:46 2025
...
def rnd_move():
    return (
        random.choice(list(unit_types.values())),
        random.choice(list(unit_types.values())),
        ...
        if combat_mod is not None:
            ...
            if player_role == "attacker":
                if mods_total > 0:
                    ...
                    challenge_dice = clip(challenge_dice[0] + abs(splits[0])), clip(
                        challenge_dice[1] + abs(splits[1])
                    )
                ...
                if mods_total < 0:
                    ...
                    splits = split_modifiers(mods_total)
                    challenge_dice = clip(challenge_dice[0] + splits[0]), clip(
                        challenge_dice[1] + splits[1]
                    )
            ...
            data = (
                ...
                attribute_mod,
                mods_total,
                original_challenge_dice,
                original_action_die,
                challenge_dice,
                action_die,
                outcomes[outcome_numeric],
                outcome_numeric,
            )
...
columns = [
    ...
    "ability_modifier",
    "total_modifier",
    "original_challende_challenge_dice",
    "original_action_die",
    "modifie_challenge_dice",
    "modified_action_dice",
    "outcome_str",
    "outcome_numeric",
]
...
hist_df[percentage_columns] = pd.DataFrame(
    hist_df["Percentages"].tolist(), index=hist_df.index
)
...
hist_df = hist_df.sort_values(
    by=["attacker", "defender", "combat_style", "role", "numbers_attacker_numeric"]
)
